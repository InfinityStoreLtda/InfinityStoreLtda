<?php
// configuracoes/empresas/receitas/editar.php - Editar empresa pagadora (receitas)
...
// Carregar registro
$stmt = $pdo->prepare('SELECT * FROM empresas_pagadoras WHERE id = :id');
$stmt->execute(['id' => $id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nome === '') {
        ...
        try {
            if ($cnpj) {
                $dup = $pdo->prepare('SELECT id FROM empresas_pagadoras WHERE cnpj = :cnpj AND id <> :id');
                $dup->execute(['cnpj' => $cnpj, 'id' => $id]);
                if ($dup->fetch()) { $erro = 'CNPJ já cadastrado para outra empresa.'; }
            }
            if (!$erro) {
                $upd = $pdo->prepare('UPDATE empresas_pagadoras 
...
<head>
  ...
  <title>Editar Empresa de Receitas</title>
...
<body>
  ...
  <div class="container">
    <h2>Editar Empresa de Receitas</h2>
  ...
  // Formatar valor inicial do CNPJ que veio do PHP
  if (cnpjInput){
  ...
  async function buscarCNPJ(){
    ...
    try{
      ...
      // preenche respeitando a opção de sobrescrever
