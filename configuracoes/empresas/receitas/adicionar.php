<?php
// configuracoes/empresas/receitas/adicionar.php - Adicionar empresa pagadora (receitas)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nome === '') {
        ...
        try {
            ...
            if ($cnpj) {
                $stmt = $pdo->prepare('SELECT id FROM empresas_pagadoras WHERE cnpj = :cnpj');
            ...
            if (!$erro) {
                $stmt = $pdo->prepare('INSERT INTO empresas_pagadoras (nome_empresa, cnpj, contato, endereco, status)
...
<title>Adicionar Empresa de Receitas</title>
...
@media (max-width: 768px) { .container { margin-left: 90px; } form { max-width: 100%; } .inline { flex-direction:column; align-items:stretch; } }
</style>
</head>
<body>
<?php include_once __DIR__ . '/../../../sidebar.php'; ?>
<div class="container">
  <h2>Adicionar Empresa de Receitas</h2>
...
// Máscara PROGRESSIVA de CNPJ (não corta a digitação)
...
// Validação por dígito verificador (CNPJ válido)
function isValidCNPJ(raw){
  const cnpj = onlyDigits(raw);
  if (cnpj.length !== 14) return false;
  if (/^(\d)\1{13}$/.test(cnpj)) return false; // rejeita repetidos (000..., 111..., etc.)
...
if (cnpjInput){
  cnpjInput.addEventListener('input', () => {
    ...
    setBadge('');
    // Ajuste simples do cursor (melhora UX)
  ...
  cnpjInput.addEventListener('blur', () => {
    ...
    if(!d){ cnpjInput.setCustomValidity(''); setHint('CNPJ é opcional. Você pode salvar sem ele.', 'info'); return; }
...
// Consulta BrasilAPI e preenche campos
async function buscarCNPJ(){
  const cnpj = onlyDigits(cnpjInput.value);

  if(!cnpj){
    setHint('Informe um CNPJ para buscar.', 'warn');
    cnpjInput.focus();
    return;
  }
  if(cnpj.length !== 14){
  ...
  try{
    ...
    // Chamada direta à BrasilAPI (CORS liberado)
    const url = `https://brasilapi.com.br/api/cnpj/v1/${cnpj}`;

    // Se preferir usar proxy no servidor, crie util/cnpj_lookup.php e troque:
    // const url = `/util/cnpj_lookup.php?cnpj=${cnpj}`;

    const resp = await fetch(url, { headers: { 'Accept':'application/json' }});
    if(!resp.ok){
      throw new Error(`Falha na consulta (HTTP ${resp.status})`);
    }
    ...
    if(nome && !nomeInput.value) nomeInput.value = nome;
    if(telefones && !contatoInput.value) contatoInput.value = telefones;
    if(enderecoFmt && !endInput.value) endInput.value = enderecoFmt;
    ...
    setHint('Dados preenchidos a partir do CNPJ. Confira antes de salvar.', 'ok');
  } catch(err){
    console.error(err);
    setBadge('erro', false);
