<?php
// receitas/adicionar.php - Adicionar receita com parcelamento
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $tipo_pagamento = $_POST['tipo_pagamento'] ?? 'unitario';
    $num_parcelas = ($tipo_pagamento == 'unitario') ? 1 : (int)($_POST['num_parcelas'] ?? 2);
    $empresa_id = !empty($_POST['empresa_id']) ? (int)$_POST['empresa_id'] : null;

    // Validar empresa_id
    if ($empresa_id) {
        $stmt = $pdo->prepare("SELECT id FROM empresas_pagadoras WHERE id = :id AND status = 'ativo'");
        $stmt->execute(['id' => $empresa_id]);
        if (!$stmt->fetch()) {
            $erro = 'Empresa inválida ou inativa.';
        }
    }

    // Validar campos obrigatórios
    if (!$erro && ($valor_total <= 0 || empty($data_inicio))) {
        $erro = 'Valor total e data da primeira parcela são obrigatórios.';
    }
    ...
    if (!$erro) {
        try {
            for ($i = 1; $i <= $num_parcelas; $i++) {
                ...
                $stmt = $pdo->prepare("INSERT INTO transacoes (tipo, descricao, valor, data, usuario_id, categoria_id, empresa_id, status_pagamento, parcela_numero, parcela_total) VALUES ('receita', :descricao, :valor, :data, :usuario_id, :categoria_id, :empresa_id, 'pendente', :parcela_numero, :parcela_total)");
                $stmt->execute([
            ...
            $erro = 'Erro ao adicionar receita: ' . $e->getMessage();
        }
    }
}
?>
