<?php
// receitas/index.php - Lista receitas com filtro de mês, quantidade por página e paginação
...
$stmt_total = $pdo->prepare("SELECT COUNT(*) FROM transacoes WHERE tipo = 'receita' AND DATE_FORMAT(data, '%Y-%m') = :mes");
$stmt_total->execute(['mes' => $mes]);
$total = $stmt_total->fetchColumn();
$paginas = ceil($total / $itens_por_pagina);

$stmt = $pdo->prepare("SELECT t.*, c.nome as categoria, e.nome_empresa as empresa, DATEDIFF(t.data, CURDATE()) as dias_vencimento FROM transacoes t LEFT JOIN categorias c ON t.categoria_id = c.id LEFT JOIN empresas_pagadoras e ON t.empresa_id = e.id WHERE t.tipo = 'receita' AND DATE_FORMAT(data, '%Y-%m') = :mes ORDER BY t.data DESC LIMIT :offset, :limit");
...
<head>
    ...
    <style>
        ...
        tr.pago-row {
            background: #e6f7e6; /* Verde claro para receitas pagas */
        }
        tr.pendente-row {
            background: #f7e6e6; /* Vermelho claro para receitas pendentes */
        }
        .pago {
            color: green;
            font-weight: bold;
...
<body>
    ...
    <div class="container">
        ...
        <div class="filtro">
            ...
            <a href="adicionar.php">Adicionar Receita</a>
        </div>
        <table>
            <tr>
                <th>Data</th>
            ...
            <?php foreach ($receitas as $receita): ?>
                <tr class="<?php echo $receita['status_pagamento'] == 'pago' ? 'pago-row' : 'pendente-row'; ?>">
                    <td><?php echo date('d/m/Y', strtotime($receita['data'])); ?></td>
                    <td><?php echo htmlspecialchars($receita['categoria'] ? ($receita['categoria'] . ' - ' . $receita['descricao']) : $receita['descricao']); ?></td>
                    <td>R$ <?php echo number_format($receita['valor'], 2, ',', '.'); ?></td>
                    <td><?php echo htmlspecialchars($receita['empresa'] ?: '-'); ?></td>
                    <td><?php echo $receita['parcela_numero'] . '/' . $receita['parcela_total']; ?></td>
                    <td class="<?php echo $receita['status_pagamento'] == 'pago' ? 'pago' : 'pendente'; ?>">
                        <?php echo $receita['status_pagamento'] == 'pago' ? 'Pago' : 'Pendente'; ?>
                    </td>
                    <td><?php echo $receita['dias_vencimento'] >= 0 ? $receita['dias_vencimento'] . ' dias' : 'Vencido'; ?></td>
                    <td>
                        <a href="editar.php?id=<?php echo $receita['id']; ?>" class="action-btn editar"><i class="fas fa-edit"></i> Editar</a>
                        <a href="excluir.php?id=<?php echo $receita['id']; ?>" class="action-btn excluir" onclick="return confirm('Confirmar exclusão?')"><i class="fas fa-trash"></i> Excluir</a>
                        <a href="atualizar_pagamento.php?id=<?php echo $receita['id']; ?>&status=<?php echo $receita['status_pagamento'] == 'pago' ? 'pendente' : 'pago'; ?>" 
                           class="action-btn status-<?php echo $receita['status_pagamento'] == 'pago' ? 'pendente' : 'pago'; ?>">
                            <i class="fas <?php echo $receita['status_pagamento'] == 'pago' ? 'fa-times' : 'fa-check'; ?>"></i> 
                            Marcar como <?php echo $receita['status_pagamento'] == 'pago' ? 'Pendente' : 'Pago'; ?>
                        </a>
                    </td>
                </tr>
            <?php endforeach; ?>
