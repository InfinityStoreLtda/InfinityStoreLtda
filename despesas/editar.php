<?php
// despesas/editar.php - Editar despesa
...
$stmt = $pdo->prepare("SELECT * FROM transacoes WHERE id = :id AND tipo = 'despesa'");
$stmt->execute(['id' => $id]);
$despesa = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($valor <= 0 || empty($data)) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE transacoes SET descricao = :descricao, valor = :valor, data = :data, categoria_id = :categoria_id, empresa_id = :empresa_id WHERE id = :id AND tipo = 'despesa'");
            $stmt->execute([
            ...
            <div class="form-group">
                ...
                <select id="empresa_id" name="empresa_id">
                    ...
                    <?php foreach ($empresas as $empresa): ?>
                        <option value="<?php echo $empresa['id']; ?>" <?php echo $empresa['id'] == $despesa['empresa_id'] ? 'selected' : ''; ?>>
            ...
            <div class="form-group">
                ...
                <select id="categoria_id" name="categoria_id">
                    ...
                    <?php foreach ($categorias as $categoria): ?>
                        <option value="<?php echo $categoria['id']; ?>" <?php echo $categoria['id'] == $despesa['categoria_id'] ? 'selected' : ''; ?>>
                            <?php echo htmlspecialchars($categoria['nome']); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
                <input type="text" name="descricao" value="<?php echo htmlspecialchars($despesa['descricao']); ?>" placeholder="Descrição adicional" style="margin-top: 5px;">
            </div>
            <div class="form-group">
                <label for="valor">Valor:</label>
                <input type="number" id="valor" name="valor" step="0.01" value="<?php echo $despesa['valor']; ?>" required>
            </div>
            <div class="form-group">
                <label for="data">Data:</label>
                <input type="date" id="data" name="data" value="<?php echo $despesa['data']; ?>" required>
            </div>
