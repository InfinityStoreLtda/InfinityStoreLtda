<?php ob_start(); ?>
<h3>Editar Contrato <?=$c['id']?></h3>
<form method="post" action="/contracts/<?=$c['id']?>/update" class="row g-3">
  <div class="col-md-6">
    <label class="form-label">Empresa Pagadora (ID)</label>
    <input name="empresa_pagadora_id" class="form-control" value="<?=htmlspecialchars($c['empresa_pagadora_id'])?>" required disabled>
  </div>
  ...
  <div class="col-12"><button class="btn btn-primary">Atualizar</button></div>
</form>
<?php
